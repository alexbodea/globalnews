...
<head>
    ...
    <link href='/themes/theme1/css/admin.css' rel='stylesheet' media='screen' />
...
<body>
    <nav class="container">
        <div class="row">
            <button class="btn">Add RSS</button>
            <button class="btn">Add Country</button>
            <button class="btn">Add Phrase</button>
        </div>
    </nav>
    ...
    <div class="container new_phrase_wrapper">
        ...
        <?php
                    ...
                    $res = mysqli_query($con,$sql);
                    while($row = mysqli_fetch_array($res)) {
                        echo '<textarea name="update_'.$row['id'].'_'.$slug.'" required>'.$row[$slug].'</textarea>';
                    }
        ?>
                    </div>
        <?php
            ...
            <button class="btn add_new_phrase">Add new phrase</button>
            <input type="submit" value="Adauga"/>
        </form>
    </div>
    <script src="/themes/theme1/js/admin.js"></script>
</body>
</html>